...
# Class Bank
class Bank(ABC): #This class must derive from class ABC
  
    def basicinfo(): #hat prints out "This is a generic bank" and returns the string "Generic bank: 0"            
...
# Class Swiss
class Swiss(Bank): #This class derive from class Bank
    def __init__(self): #a constructor that initializes a class variable `bal` to 1000
        self.bal = 1000
    """ 
    ...
    def withdraw(self, amount): #amount represents the amount to be withdrawn from the bank           
        if amount > self.bal : #Since amount is greater than balance, 
            #then do not deduct any money from the class variable `bal'                 
            print("Insufficient funds")
            return self.bal
        else:
            self.bal -= amount #Deduct the withdrawn amount from the bank bal
            print(f"Withdrawn amount: {amount}") 
            print(f"New balance: {self.bal}") #the new  balance 
            return self.bal
